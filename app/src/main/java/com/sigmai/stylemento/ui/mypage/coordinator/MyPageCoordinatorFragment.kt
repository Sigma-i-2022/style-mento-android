...
class MyPageCoordinatorFragment(private var showMenu: Int) :
    BaseFragment<FragmentMyPageCoordinatorBinding>() {
...
//            .into(binding.myPageCoordinatorImg)
    ...
    fun reply(position: Int) {
        ...
        binding.myPageCoordinatorReplySendText.setOnClickListener(View.OnClickListener {
            Client.addReviewItemAt(
                ReviewItem(
                    ReviewType.REPLY,
                    Client.getCoordinatorInfo().nickname,
                    content
                ), position
            )
            binding.myPageCoordinatorReplyLayout.visibility = View.GONE
