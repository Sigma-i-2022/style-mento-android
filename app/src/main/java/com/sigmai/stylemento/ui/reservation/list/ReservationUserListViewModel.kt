...
@HiltViewModel
class ReservationUserListViewModel @Inject constructor() : ViewModel(), ReservationUserListener {
    @Inject
    ...
    override fun onAccept(view: View, seq: Long, email: String, time: String, position: Int) {
        viewModelScope.launch {
            withContext(Dispatchers.IO){
                reservationRepository.postReservationCrdiFix(email, seq, time)
                loadData(position)
            }
        }
    }
    ...
    override fun onDelete(view: View, seq: Long, email: String) {
        ...
        view.findNavController().navigate(R.id.action_reservation_user_list_page_to_reservation_cancel_user_page, bundle)
    }
}