...
class MyPageWorkFragment(private val owner : Coordinator) : BaseFragment<FragmentMyPageWorkBinding>() {
    override val layoutResourceId = R.layout.fragment_my_page_work

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)

        val workAdapter = CoordinatorWorkAdapter(this, owner)
        val gridLayoutManager = GridLayoutManager(context,3, GridLayoutManager.VERTICAL, false)

        workAdapter.setDataSet(owner.workItems)
    ...
    private fun ownerCheck(){
        if(owner.email == Client.getCoordinatorInfo().email){
            ...
            binding.myPageWorkAddImg.visibility = View.GONE
        }
    }
}